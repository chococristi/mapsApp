...
public class SwitchCustom: UIControl {
    ...
    var thumbView = SwitchCustomThumbView(frame: CGRect.zero)
...
extension SwitchCustom {
    ...
    func setOn(onStatus:Bool, animated:Bool) {

        switch animated {
        case true:
            self.animate(onStatus: onStatus)
        case false:
            self.isOn = onStatus
    ...
    fileprivate func animate(onStatus:Bool? = nil) {
        self.isOn = onStatus ?? !self.isOn
